...
CONN_STR=([DAEG]='jdbc:db2://ukabbeydbt1.perplex.ibm.anplc.co.uk:5027/UKABBEYDBT1' [DAUG]='jdbc:db2://ukabbeydapg.perplex.ibm.anplc.co.uk:5040/UKABBEYDAPG' [DHEG]='jdbc:db2://choq.cahoot.pre.corp:5002/DHTG' [DUPG]='jdbc:db2://comq.cbuk.pre.corp:5002/DUQG' )
DB=`grep -i ${5}, /santanderukretail/ControlM/mainframe/scripts/tables.txt |grep -i ${2}| awk -F, '{print $3}'`
#FUN_GRP=`grep ${5} tables.txt | awk -F, '{print $3}'`
...
RUTA="/retailuk/landing/${DIR}/${TABLA_DIR}/data_date_part=${ANO}-${MES}-${DIA}"
sleep 1m
...
--target-dir ${RUTA} -where "${CAMPO} = '${FECHA_INC_INI}'" -m ${HILOS}
