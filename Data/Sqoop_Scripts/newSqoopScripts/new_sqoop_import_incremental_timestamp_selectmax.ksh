...
passid=$8
entity=$9
echo "CURRENT_TIMESTAMP Passed By ControlM ${CURRENT_TIMESTAMP}"
...
if [ -n "$maxValue" ]; then
echo "Inside if block :  maxValue is not null"
        varWhere="\"${CAMPO}>'${maxValue}' AND ${CAMPO}<'${CURRENT_TIMESTAMP}'\""
else
echo "Inside else block :  maxValue is null"	
	varWhere="\"${CAMPO}<'${CURRENT_TIMESTAMP}'\""
fi

echo "var where :$varWhere"
#exit 0 
...
--target-dir ${RUTA} -where "$varWhere" -m ${HILOS} 
code=$?
